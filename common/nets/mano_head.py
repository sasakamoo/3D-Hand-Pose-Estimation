...
#
# BUGS FIXED:
#   1. Double-subtraction of th_hands_mean from GT pose removed.
#      FreiHAND training_mano.json already stores pose as residual from mean,
#      so subtracting again corrupts all GT joint angles.
#   2. pred_verts / pred_joints coordinate frame clarified in comments:
#      ManoLayer outputs in MANO local space (wrist ~ origin), already /1000 → metres.
#      Do NOT subtract mano_root from these outside this module.
# ------------------------------------------------------------------------------
...
class ManoHead(nn.Module):
    ...
    def forward(self, pose6d, shape, mano_params=None):
        """
        pose6d      : (L, N, B, 6)  — L decoder layers, N queries, B batch, 6D rot
        shape       : (L, B, 10)    — shape per layer (or similar)
        mano_params : (B, 58)       — GT pose(48) + shape(10) from FreiHAND JSON
                                      Already stored as residual from mean hand pose.

        Outputs (pred_mano_results, gt_mano_results):
            verts3d  : (L, B, 778, 3)  metres, MANO local frame (wrist ~ origin)
            joints3d : (L, B, 21,  3)  metres, MANO local frame
        NOTE: these are NOT in camera space. Do not add/subtract mano_root externally.
        """
        ...
        pred_verts  = pred_verts.view(L, B, pred_verts.shape[1], pred_verts.shape[2]) / 1000
        pred_joints = pred_joints.view(L, B, pred_joints.shape[1], pred_joints.shape[2]) / 1000
        ...
        if mano_params is not None:
            gt_mano_shape = mano_params[:, self.mano_pose_size:]          # (B, 10)
            gt_mano_pose  = mano_params[:, :self.mano_pose_size].clone()  # (B, 48)

            # FIX: Do NOT subtract th_hands_mean here.
            # FreiHAND training_mano.json stores pose already as the residual
            # from the mean hand pose. Subtracting again double-removes the mean
            # and corrupts all GT joint angles.
            # Original (wrong) line was:
