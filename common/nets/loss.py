...
#
# BUGS FIXED:
#   1. JointHeatmapLoss.__ini__ typo → __init__  (super().__init__ was never called)
#   2. ManoLoss.forward used uninitialized local variables — if any lambda/key
#      condition was False the return statement raised UnboundLocalError.
#      All losses now initialised to zero tensors before the conditional blocks.
# ------------------------------------------------------------------------------
...
class JointHeatmapLoss(nn.Module):
    # FIX: was __ini__ (typo) — super().__init__() was never called
    def __init__(self):
...
class ManoLoss(nn.Module):
    ...
    def forward(self, preds, gts):
        # FIX: initialise all losses to zero so the return statement never
        # references an unbound variable when a lambda or key condition is False.
        device = preds["verts3d"].device
        mesh3d_loss      = torch.zeros(1, device=device)
        joints3d_loss    = torch.zeros(1, device=device)
        pose_param_loss  = torch.zeros(1, device=device)
        shape_param_loss = torch.zeros(1, device=device)
        shape_regul_loss = torch.zeros(1, device=device)
        pose_regul_loss  = torch.zeros(1, device=device)
        ...
        if self.lambda_regulpose:
            pose_regul_loss = self.lambda_regulpose * F.mse_loss(
                preds["mano_pose"][:, 3:],
                torch.zeros_like(preds["mano_pose"][:, 3:]),
            )
