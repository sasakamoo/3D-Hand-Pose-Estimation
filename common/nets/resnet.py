...
class ResNetBackbone(nn.Module):
    ...
    def init_weights(self):
        import os, torch, torchvision.models as tvm
        # Use cached weights from scratch — no internet needed on GPU nodes
        torch.hub.set_dir("/scratch/kghasemz/torch_hub")
        _builders = {
        ...
        try:
            org_resnet = _builders[self.name](weights=_weights[self.name]).state_dict()
            org_resnet.pop("fc.weight", None)
            org_resnet.pop("fc.bias", None)
            self.load_state_dict(org_resnet)
            print(f"Initialized {self.name} from local cache")
